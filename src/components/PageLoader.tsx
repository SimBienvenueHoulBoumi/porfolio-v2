    ...
    <div className="fixed inset-0 z-[9999] overflow-hidden">
      <div
        className={`absolute inset-0 transition-colors duration-300 ${
          isAurora ? "bg-slate-50" : "bg-gray-950"
        }`}
      />
      <div
        className={`pointer-events-none absolute inset-0 hidden mix-blend-screen opacity-70 sm:block ${
          isAurora
            ? "bg-[radial-gradient(circle_at_center,_rgba(148,163,184,0.16)_0%,_rgba(148,163,184,0.06)_55%,_transparent_85%)]"
            : "bg-[radial-gradient(circle_at_center,_rgba(34,197,94,0.22)_0%,_rgba(15,23,42,0.9)_55%,_transparent_85%)]"
        }`}
      />
      <div className="absolute inset-0">
        <div
          className={`absolute left-1/2 top-1/2 h-48 w-48 -translate-x-1/2 -translate-y-1/2 rounded-full blur-2xl sm:h-72 sm:w-72 sm:blur-3xl ${
            isAurora ? "bg-emerald-100/40" : "bg-emerald-500/20"
          }`}
        />
      </div>
      ...
      <div className="relative flex h-full w-full flex-col items-center justify-center px-4 py-6 sm:px-6 sm:py-8">
        <div
          ...
          className={`flex w-full max-w-sm flex-col items-center gap-4 rounded-3xl border px-6 py-8 text-center shadow-2xl transition-colors sm:px-8 sm:py-10 ${
            isAurora
              ? "border-emerald-200/70 bg-white/85 text-slate-700 shadow-emerald-200/40"
              : "border-emerald-500/25 bg-slate-900/80 text-emerald-300 shadow-emerald-500/25"
          }`}
        >
            ...
            <div className="relative h-16 w-16">
              <div
                className={`absolute inset-0 animate-spin-slow rounded-full border-[6px] border-transparent ${
                  isAurora ? "border-t-emerald-500 border-r-emerald-400" : "border-t-emerald-400 border-r-emerald-300"
                }`}
              />
              <div
          ...
          <p
            className={`text-sm font-medium tracking-wide ${
              isAurora ? "text-slate-700" : "text-emerald-200"
            }`}
          >
            {copy.message}
          </p>
            ...
            <span className="flex gap-1">
              {[0, 1, 2].map((dot) => (
                <span
                  ...
                  isAurora ? "bg-emerald-500/80" : "bg-emerald-400/80"
                  }`}
                />
              ))}
            </span>
          ...
          {showSupport && (
            <p
              className={`max-w-xs text-xs ${
                isAurora ? "text-slate-700" : "text-slate-300"
              }`}
            >
              {copy.fallback}
            </p>
        ...
        {showRetry && (
          <button
            ...
            className={`mt-4 inline-flex items-center justify-center rounded-full px-4 py-2 text-xs font-semibold transition-all ${
              isAurora
                ? "bg-emerald-500/90 text-white hover:bg-emerald-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/60"
                : "bg-emerald-500/80 text-slate-950 hover:bg-emerald-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/60"
            }`}
          >
            {copy.retryLabel}
          </button>
